...
exports.createMissing = asyncHandler(async (req, res) => {
  const { name, age, gender, lastSeenText, lat, lng, dateMissing, photos: bodyPhotos } = req.body;
  // Use photos from request body (Cloudinary URLs) or from uploaded files
  const photos = bodyPhotos && Array.isArray(bodyPhotos) 
    ? bodyPhotos 
